...
function BackendStatus({ state }: { state: ClientState | null }) {
  const b = state?.backend;
  const configured = b?.configured;
  return (
    <span className="inline-flex items-center gap-2 text-xs text-muted">
      <span className={`h-2.5 w-2.5 rounded-full ${configured ? 'bg-mint' : 'bg-coral'}`} />
      Runware {configured ? 'ready' : 'no API key'}
      {b?.model && <span className="chip bg-surface-2 ml-2">{b.model}</span>}
    </span>
  );
}
