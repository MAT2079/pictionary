...
export default function SettingsPanel({ state }: { state: ClientState }) {
  ...
  function patchRunware(p: Partial<Settings['runware']>) {
    setSettings((s) => (s ? { ...s, runware: { ...s.runware, ...p } } : s));
  }
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      {/* Backend — Runware API */}
      <Group title="Backend — Runware">
        <Row label="Status">
          <span className="text-sm">
            {state.backend?.configured ? '🟢 API key configured' : '🔴 no API key (set RUNWARE_API_KEY)'}
          </span>
        </Row>
        <Row label="Model AIR">
          <input className="input" value={settings.runware.model} placeholder="runware:100@1"
            onChange={(e) => patchRunware({ model: e.target.value })} />
        </Row>
        <Row label="Scheduler (optional)">
          <input className="input" value={settings.runware.scheduler} placeholder="model default"
            onChange={(e) => patchRunware({ scheduler: e.target.value })} />
        </Row>
        <Row label="API key override (session only)">
          <input className="input font-mono text-xs" type="password" value={settings.runware.apiKey}
            placeholder="leave blank to use RUNWARE_API_KEY env"
            onChange={(e) => patchRunware({ apiKey: e.target.value })} />
        </Row>
        <TestRunware />
        <p className="text-xs text-muted">Browse model AIR identifiers at my.runware.ai/models. The key is never sent back to the browser.</p>
      </Group>
...
function TestRunware() {
  const [state, setState] = useState<'idle' | 'testing' | 'ok' | 'fail'>('idle');
  const [msg, setMsg] = useState('');
  async function run() {
    setState('testing'); setMsg('');
    try {
      const r = await api.testRunware();
      if (r.ok) { setState('ok'); setMsg(`Got ${r.images} image(s).`); }
      else { setState('fail'); setMsg(r.error ?? 'failed'); }
    } catch (e) { setState('fail'); setMsg((e as Error).message); }
  }
  return (
    <div className="flex items-center gap-3">
      <button className="btn-ghost" onClick={run} disabled={state === 'testing'}>
        {state === 'testing' ? 'Testing…' : 'Test generation'}
      </button>
      {state === 'ok' && <span className="text-mint text-sm">🟢 {msg}</span>}
      {state === 'fail' && <span className="text-coral text-sm">🔴 {msg}</span>}
    </div>
  );
}
