...
import { isConfigured } from './runware.js';
...
function backendStatus() {
  return { provider: 'runware', configured: isConfigured(), model: state.settings.runware.model };
}
...
export function serializeFor(role: Role, teamId?: string) {
  ...
  if (role === 'operator') {
    return {
      ...
      // Whether an API key is present at all (never send the key itself).
      runwareConfigured: isConfigured(),
